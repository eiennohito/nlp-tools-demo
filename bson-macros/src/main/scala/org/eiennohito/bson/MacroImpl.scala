...
import org.eiennohito.bson.Macros2.Options._
...
private object MacroImpl {
  def reader[A: c.WeakTypeTag, Opts: c.WeakTypeTag](
      c: whitebox.Context): c.Expr[BSONDocumentReader[A]] =
    c.universe.reify(new BSONDocumentReader[A] {
  ...
  def writer[A: c.WeakTypeTag, Opts: c.WeakTypeTag](
      c: whitebox.Context): c.Expr[BSONDocumentWriter[A]] =
    c.universe.reify(new BSONDocumentWriter[A] {
  ...
  def handler[A: c.WeakTypeTag, Opts: c.WeakTypeTag](
      c: whitebox.Context): c.Expr[BSONDocumentHandler[A]] =
    c.universe.reify(
  ...
  private def Helper[A: c.WeakTypeTag, Opts: c.WeakTypeTag](c: whitebox.Context) =
    new Helper[c.type, A](c) {
      val A = c.weakTypeOf[A]
      val Opts = c.weakTypeOf[Opts]
    }

  private abstract class Helper[C <: whitebox.Context, A](val c: C) extends ImplicitResolver[C] {
    import c.universe._
    ...
    lazy val readBody: c.Expr[A] = {
      ...
      if (hasOption[Macros2.Options.Verbose]) {
    ...
    private def readBodyFromImplicit(tpe: Type)(r: Type => Implicit): Option[Tree] = {
      ...
      else if (!hasOption[Macros2.Options.AutomaticMaterialization]) {
        c.abort(
          c.enclosingPosition,
          s"Implicit not found for '${tpe.typeSymbol.name}': ${classOf[Reader[_]].getName}[_, ${tpe.typeSymbol.fullName}]")
      } else None
    }
    ...
    private def readBodyConstructSingleton(implicit tpe: Type) = {
      val sym = tpe match {
        ...
        case _ =>
          c.abort(
            c.enclosingPosition,
            s"Something weird is going on with '$tpe'. Should be a singleton but can't parse it")
      }
    ...
    private def readBodyConstructClass(implicit tpe: Type) = {
      ...
      val comp = tpe.companion

      val values = constructor.paramLists.headOption.toSeq.flatten.zipWithIndex.map { case (param, i) =>
        ...
        if (reader.isEmpty) {
          c.abort(c.enclosingPosition,
                  s"Implicit not found '$pname': ${classOf[Reader[_]].getName}[_, $sig]")
        }

        if (param.annotations.exists(_.tree.tpe =:= typeOf[Flatten])) {
          if (reader.toString == "forwardReader") {
            c.abort(c.enclosingPosition, s"Cannot flatten reader for '$pname': recursive type")
          }

          if (!(reader.tpe <:< appliedType(docReaderType, List(sig)))) {
            c.abort(c.enclosingPosition,
                    s"Cannot flatten reader '$reader': doesn't conform BSONDocumentReader")
          }

          q"$reader.read(document)"
        } else {
          val defParamName = TermName("apply$default$" + (i + 1))
          comp.member(defParamName) match {
            case x: MethodSymbol =>
              opt match {
                case Some(_) => q"document.getAsUnflattenedTry($pname)($reader).getOrElse($x)"
                case _       => q"document.getAsTry($pname)($reader).getOrElse($x)"
              }
            case _ =>
              opt match {
    ...
    private def writeBodyFromImplicit(id: Ident, tpe: Type)(r: Type => Implicit): Option[Tree] = {
      ...
      if (!writer.isEmpty) {
        ...
        c.abort(
          c.enclosingPosition,
          s"Implicit not found for '${tpe.typeSymbol.name}': ${classOf[Writer[_]].getName}[_, ${tpe.typeSymbol.fullName}]")
    ...
    private def writeBodyConstructClass(id: Ident, tpe: Type): Tree = {
      ...
      val (optional, required) =
        constructorParams.zipWithIndex
          .zip(types)
          .filterNot {
      ...
      def resolveWriter(pname: String, tpe: Type) = {
        ...
        if (writer.isEmpty) {
          c.abort(c.enclosingPosition,
                  s"Implicit not found for '$pname': ${classOf[Writer[_]].getName}[$tpe, _]")
        }
      ...
      val tupleElement: Int => Tree = {
        ...
        if (types.length == 1) { _: Int =>
          tuple
        } else { i: Int =>
          Select(tuple, TermName("_" + (i + 1)))
        }
      }

      val bufName = TermName("buf")

      def mustFlatten(param: Symbol, pname: String, sig: Type, writer: Tree): Boolean = {
        if (param.annotations.exists(_.tree.tpe =:= typeOf[Flatten])) {
          if (writer.toString == "forwardWriter") {
            c.abort(c.enclosingPosition, s"Cannot flatten writer for '$pname': recursive type")
          }

          if (!(writer.tpe <:< appliedType(docWriterType, List(sig)))) {
            c.abort(c.enclosingPosition,
                    s"Cannot flatten writer '$writer': doesn't conform BSONDocumentWriter")
          }
      ...
      val extra = optional.map {
        case ((param, i), optType) =>
          ...
          val vp = ValDef(Modifiers(c.universe.Flag.PARAM), vterm, TypeTree(sig), EmptyTree) // ${v} =>
    ...
    private def classNameTree(tpe: c.Type): Option[c.Expr[(String, BSONString)]] = {
      ...
      if (tpeSym.isSealed && tpeSym.isAbstract) Some {
        val name = if (hasOption[Macros2.Options.SaveSimpleName]) {
    ...
    private def parseUnionTypes: Option[List[c.Type]] = {
      val unionOption = c.typeOf[Macros2.Options.UnionType[_]]
      val union = c.typeOf[Macros2.Options.\/[_, _]]
          ...
          case tree :: rem =>
            if (tree <:< union) {
              tree match {
                ...
                case _ => c.abort(c.enclosingPosition, s"Union type parameters expected: $tree")
              }
        ...
        path.headOption match {
          case Some(cls: ClassSymbol)
            ...
            val newSub: Set[Type] = if ({
              ...
              applyMethod(tpe).isEmpty || unapplyMethod(tpe).isEmpty
            }) {
              ...
              c.warning(c.enclosingPosition,
                        s"cannot handle class ${cls.fullName}: type parameter not supported")
              Set.empty
          ...
          case Some(o: ModuleSymbol)
            ...
            val newSub: Set[Type] = if (!o.moduleClass.asClass.isCaseClass) {
              c.warning(c.enclosingPosition,
                        s"cannot handle object ${o.fullName}: no case accessor")
              Set.empty
      ...
      types.headOption match {
        case Some((TypeRef(NoPrefix, a, _), TypeRef(NoPrefix, b, _))) => // for generic parameter
          if (a.fullName != b.fullName) {
            c.warning(c.enclosingPosition, s"Type symbols are not compatible: $a != $b")

            false
          } else conforms(types.tail)

        case Some((a, b)) if a.typeArgs.size != b.typeArgs.size =>
          c.warning(c.enclosingPosition, s"Type parameters are not matching: $a != $b")
        ...
        case Some((a, b)) if a.typeArgs.isEmpty =>
          if (a =:= b) conforms(types.tail)
            ...
            c.warning(c.enclosingPosition, s"Types are not compatible: $a != $b")

            false
          }

        case Some((a, b)) if a.baseClasses != b.baseClasses =>
          c.warning(c.enclosingPosition, s"Generic types are not compatible: $a != $b")
    ...
    /**
      ...
      */
    private def matchingApplyUnapply(implicit tpe: Type): Option[(MethodSymbol, MethodSymbol)] =
      for {
        ...
        apply <- alternatives.find { alt =>
          alt.paramLists match {
            case params :: ps
                if ps.isEmpty || ps.headOption.flatMap(_.headOption).exists(_.isImplicit) =>
            ...
            case _ =>
              c.warning(
                c.enclosingPosition,
                s"""Constructor with multiple parameter lists is not supported: ${tpe.typeSymbol.name}${alt.typeSignature}""")
  ...
  sealed trait ImplicitResolver[C <: whitebox.Context] {
    val c: C
     ...
     */
    @annotation.tailrec
    private def refactor(boundTypes: Map[String, Type])(
        in: List[Type],
        base: TypeSymbol,
        out: List[Type],
        ...
        filter: Type => Boolean,
        replacement: Type,
        altered: Boolean): (Type, Boolean) = in match {
      case tpe :: ts =>
        boundTypes.getOrElse(tpe.typeSymbol.fullName, tpe) match {
          case t if filter(t) =>
            refactor(boundTypes)(ts, base, replacement :: out, tail, filter, replacement, true)

          case TypeRef(_, sym, as) if as.nonEmpty =>
            refactor(boundTypes)(as,
                                 sym.asType,
                                 List.empty,
                                 (ts, base, out) :: tail,
                                 filter,
          ...
          case t => refactor(boundTypes)(ts, base, t :: out, tail, filter, replacement, altered)
        }
      ...
      case _ =>
        ...
        tail match {
          case (x, y, more) :: ts =>
            refactor(boundTypes)(x, y, tpe :: more, ts, filter, replacement, altered)
    ...
    /**
      ...
      */
    private def normalized(boundTypes: Map[String, Type])(tpe: Type): (Type, Boolean) =
      boundTypes.getOrElse(tpe.typeSymbol.fullName, tpe) match {
        case t if t =:= A => PlaceholderType -> true

        case TypeRef(_, sym, args) if args.nonEmpty =>
          refactor(boundTypes)(args,
                               sym.asType,
                               List.empty,
                               List.empty,
                               _ =:= A,
    ...
    private class ImplicitTransformer(boundTypes: Map[String, Type], forwardSuffix: String)
        extends Transformer {
    ...
    private def prettyType(boundTypes: Map[String, Type])(t: Type): String =
      boundTypes.getOrElse(t.typeSymbol.fullName, t) match {
        case TypeRef(_, base, args) if args.nonEmpty =>
          s"""${base.asType.fullName}[${args.map(prettyType(boundTypes)(_)).mkString(", ")}]"""

        case t => t.typeSymbol.fullName
      }

    protected def resolver(boundTypes: Map[String, Type], forwardSuffix: String)(
        tc: Type): Type => Implicit = {
